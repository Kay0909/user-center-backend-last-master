...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword, String planetCode) {
        ...
        if (userAccount.length() < 3 || userAccount.length() > 20) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户账号应该大于3位小于20位");
        }
        if (userPassword.length() < 6 || checkPassword.length() < 6 || userPassword.length() > 20 || checkPassword.length() > 20) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户密码应该大于6位小于20位");
        }
